...
- **Attempt UI** (`features/attempt-ui/`): `AttemptTopbar` (`leading` slot for mobile), `AttemptSummaryStrip`, `AttemptActionBar` (`legacy`, `compactMobile`), `LiveTimeLeft` (`variant="inline"`), `InfoStrip`, etc. Used by BOTH full-length exam and daily-test attempt — **change once, both update**. Exam shell: `layout/exam/{ExamShell,Topbar,TimerStrip,Sidebar}`.
