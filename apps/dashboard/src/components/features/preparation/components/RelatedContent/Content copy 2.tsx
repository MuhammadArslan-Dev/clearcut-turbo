...
interface Props {
  tab: string;
  notes?: ContentType[] | null;
  bonusVideos?: ContentType[] | null;
  concepts?: ContentType[] | null;
...
export function Notes({ notes }: { notes?: ContentType[] | null }) {
  ...
  const handleDownload = (note: ContentType) => {
  ...
  const filterNotes = useMemo(() => {
    ...
    if (selectedSection?.type === "subject") {
      const languageNotes = notes.filter(
        (note) => note?.primary_language === language,
      );
                ...
                <div className="flex flex-col items-center gap-4 px-4 py-3">
                  ...
                  <div className="w-full px-6">
                    ...
                    >
                      <Button
                        onClick={() => {
                          handleDownload(filterNotes as ContentType);
                          markVideoWatch();
                        }}
                        sx={{
                          padding: "2px 12px",
...
export function BonusVideo({ videos }: { videos?: ContentType[] | null }) {
        ...
        <div className="grid md:grid-cols-3 gap-4">
          {videos?.map((video, i) => (
            <motion.div key={i} whileHover={{ scale: 1.02 }}>
              <VideoCard url={video?.content_link!} />
            </motion.div>
          ))}
        </div>
      </div>
    ...
    >
      <div className="space-y-4">
        ...
        <motion.div
          ...
          className="flex md:flex-row justify-between flex-col gap-2 md:gap-6"
        >
          {[0, 1].map((col) => (
            <div key={col} className="space-y-2 w-full">
              {Array.from({ length: 5 }).map((_, i) => (
                <motion.div
                  ...
                  className={clsx(
                    " cursor-pointer flex flex-col gap-1  py-2",
                    i === 2 &&
                      "rounded-r-md bg-brand/7 border-l-4 border-brand",
                    i === 2 ? "pl-[9px] pr-3" : "pl-3",
                  )}
                >
                  <div className="flex gap-6 items-center ">
                    <StatusChip
                      variant="outline"
                      tone="info"
                      className="!text-brand border-none px-3 py-1 !bg-brand/9 body-medium !font-semibold"
                      label="12:30"
                    />
                    <Text
                      as="p"
                      variant="body-medium"
                      color={i === 2 ? "primary-normal" : "gray-normal"}
                      weight={i === 2 ? "semibold" : "normal"}
                    >
                      Click on any concept for
                    </Text>
                  </div>
                  {i === 2 && (
                    <Text
                      ...
                      className="text-left"
                    >
                      faster learning and revision Lorem ipsum dolor sit amet
                      consectetur adipisicing elit. Eveniet, iste suscipit culpa
                      maiores a quae fugiat doloremque nulla cum omnis.
                      Necessitatibus dolor illum alias delectus laboriosam. Qui
                      voluptatum necessitatibus commodi.
                    </Text>
                  )}
                </motion.div>
              ))}
