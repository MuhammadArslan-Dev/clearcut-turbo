...
export default function RelatedContentWrapper() {
  const { selectedTopic, selectedChapter, loading } = usePreparationStore();
  ...
  const notes = React.useMemo(
    () => safeContent.filter((i) => i.content_flag === "notes"),
    [safeContent],
  );

  const bonusVideos = React.useMemo(
    () => safeContent.filter((i) => i.content_flag === "supporting_video"),
    [safeContent],
  );
  ...
  const availableTabs = React.useMemo(() => {
    ...
    return tabs;
  }, [hasNotes, hasBonusVideos, hasTrends, hasConcepts]);
    ...
    <div className="md:rounded-md overflow-hidden">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          {tab === "concepts" && <Content tab="concepts" concepts={concepts} />}
        </motion.div>
      </AnimatePresence>
    </div>
  );
