...
export default function ChangePaperModal() {
  ...
  useBackHandler({
    isOpen,
    onClose: () => closeModal("change-paper-modal"),
  });
  ...
  const handleChangePaper = () => {
    const foundPaper = papers?.find((p) => p?.id === Number(selected)) ?? null;

    setPaper(foundPaper);
    closeModal("change-paper-modal");
  };
  ...
  return (
    <AnimatePresence>
      {isOpen && active === "change-paper-modal" && (
        <Container
          key="change-paper-modal"
          isHeader={false}
        ...
        >
          <div className="bg-white h-full flex flex-col max-h-[90vh] overflow-y-auto justify-between">
            <Header onClose={() => closeModal("change-paper-modal")} />
