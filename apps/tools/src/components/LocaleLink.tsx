 ...
 *
 * On a Hindi or Marathi page this renders a plain <a> with the full absolute
 * "/hi/tools/{tool}/..." (or "/mr/tools/{tool}/...") path instead of
 * next/link, for two reasons: (1) next/link would auto-prepend basePath
 * ("/tools") to a "/hi/..." href, landing on "/tools/hi/..." instead of the
 * public "/hi/tools/{tool}/..." one (see worker/src/index.ts's HI_PREFIX
 * comment); (2) the browser's actual URL on a non-English page doesn't match
 * the app's configured basePath at all, which next/link's client-side
 * router isn't built to navigate correctly from. A plain <a> sidesteps both
 * — full page load, but this is a static export of simple content pages, so
...
export default function LocaleLink({
  ...
  if (locale !== "en") {
    return (
      <a href={`/${locale}/tools/${tool}${suffix}`} className={className}>
        {children}
      </a>
    );
  }
