...
| `apps/tools` | Next.js 16, static export (`output: "export"`), Tailwind v4 | Standalone browser tools (photo/signature resizer, age eligibility calculator, Syllabus Tracker — anonymous/localStorage by default, with an optional login-to-save for the Syllabus Tracker, see "Tools: Save for Future") deployed to Cloudflare Pages, fronted by a Cloudflare Worker at `clearcutoff.in/tools/*` — see "apps/tools deployment" below |
| `packages/*` | TypeScript, no build step (consumed as source via `exports` maps) | Shared code — see table below |

`apps/blog`, `apps/dashboard`, and `apps/landing` are Next.js 16 App Router with i18n via `next-intl` (`en` default unprefixed, `hi` under `/hi`, config in `packages/i18n`) and Tailwind v4. `apps/tools` is intentionally outside this — no `next-intl`, no CMS, no dashboard handoff; its only runtime backend use is the Syllabus Tracker's public syllabus reads plus the optional login-to-save (see "Tools: Save for Future").
...
| `packages/auth` | `createAuthFeature(config)` — a factory each app calls once (in its own `src/lib/auth.ts`) with app-specific `apiClient`/`apiBaseUrl`/`redirectBaseUrl`/`onEvent`, returning `AuthProvider`, `AuthModal`, `useAuthStore`, `useAuthModal`, `InlineAuthFlow`, `authApi`. Never import `@clearcut/auth` directly from app code — always through that app's `lib/auth.ts` (tools: `src/lib/toolsAuthFeature.ts`; it may also import the tiny pure modules `@clearcut/auth/token` and `@clearcut/auth/verification-events`). **The `Verification Sent` / `Verification Resent` events are defined once in `verification-events.ts`** — every OTP entry point (the three package screens and landing's own `StartAuthForm`) calls `trackVerificationSent` / `trackVerificationResent` instead of building the payload (`user_phone`, `verification_method`, `verification_mode: "SMS"`, `verification_purpose: "Login"`, plus the running `resend_count`). Sent fires only after the OTP-send API succeeded; a resend must never fire Sent. Tools has no Amplitude of its own beyond `src/lib/toolsAnalytics.ts` (lazy, login modal only). **`Authentication Outcome`** (`outcome: successful|failed`, `auth_method: phone_otp`, `failure_reason` only on failure) is also defined there (`trackAuthSuccess` / `trackAuthFailure`, once per verify attempt), and the optional `onIdentify` config calls each app's Amplitude `setUserId` with the backend user uuid — for a brand-new signup right after the OTP is sent, for everyone after a successful verify — so first-touch `initial_utm_*` (collected by the Amplitude web SDK, not by us) attaches to that user. **`Signed Up` is server-side only** (`AuthController::login()` → `AmplitudeService::track(AmplitudeEvent::SignedUp, …)`, backend repo; needs `AMPLITUDE_ENABLED=true` + `AMPLITUDE_API_KEY` there). Blog + landing use it with the default post-login redirect to the dashboard; tools passes `onAuthenticated` so login stays on the page. |
...
### Tools: Save for Future (login without leaving Tools)

The Syllabus Tracker keeps working anonymously on `localStorage` exactly as before. A visitor can additionally log in (phone → OTP, the shared `@clearcut/auth` modal) and tap **Save for Future** to store a tracker on their account (`user_syllabus_trackers`, `GET/POST/DELETE /tools/syllabus-tracker`, `auth:sanctum`). Rules that must keep holding:

- **Login never redirects.** `createAuthFeature({ onAuthenticated })` (see `packages/auth/src/screens/types.ts`) replaces the `window.location.replace(dashboard…)` in the OTP/Truecaller screens with a callback; omitting it (blog/landing) keeps the original redirect. `AuthProvider` is deliberately **never mounted in tools** — its mount effect redirects any logged-in visitor to the dashboard. `LoginCopy` (optional `AuthModal` prop) lets an app override the modal's title/subtitle/CTA/trial box without touching other apps' copy.
- **The account copy stores ids + progress only** (`user_syllabus_trackers`: `exam_id`, `root_navigation_id`, `navigation_id` — NULL means unscoped — and a `progress` JSON of done chapter ids per section). Exam/level/subject/chapter names, logo and order are derived from the *current* syllabus by `SyllabusTreeService` + `SyllabusTrackerMapper` and follow `?locale=en|hi|mr` on the tracker GET/POST, so a syllabus edit or new language needs no tracker migration. The public `/tools/syllabus/*` endpoints and the tracker mapping share that one service.
- **The login UI is lazy.** `toolsAuthFeature.ts` (axios, zustand, OTP screens) is only reachable through `ToolsAuthGate`, loaded with `next/dynamic` on the first login request, so the anonymous tracker's bundle never contains it. Tools' main bundle only imports `@clearcut/auth/token` (`toolsSession.ts`) and uses plain `fetch` (`lib/api/syllabusTrackerApi.ts`).
- **Server data never overwrites local data on its own.** Saving is explicit (button click, or a click that needed a login first and resumes after it) and never on chapter toggles. Loading a saved tracker is an explicit "Load" (with a confirm when it would replace a *different* local copy). Saving over an account copy that differs from local *and* that this browser didn't last write asks first (`cc-syllabus-tracker-synced-v1` remembers the fingerprint this browser last synced; `progressFingerprint()` compares progress independent of subject order).
- Session token = the shared `CSRF_TOKEN` localStorage key. In production tools shares the `clearcutoff.in` origin with landing, so a user who logs in from Tools is bounced to the dashboard if they later open Landing (landing's existing behaviour).
- Known dev-only cosmetic: the shared login modal's flag image uses `next/image unoptimized` with an absolute `/images/indian-flage.webp`, which ignores tools' `basePath` — it 404s under `next dev`/`*.pages.dev` but resolves in production (landing serves that path on the same origin).
